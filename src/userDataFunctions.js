...
export const getUserData = async (
  ...
  //user wallet balance
  const userWalletBalance = await web3.eth.getBalance(metaMastAcc);
  ...
  await dcentralized.methods
    ...
    .then((val) => {
      ...
      store.dispatch({
        type: "DLT_CURRENT_PRICE",
        payload: web3.utils.fromWei(val),
      });

      store.dispatch({
        type: "WITHOUT_DLT_CURRENT_PRICE",
        payload: val,
      });
    });
...
export const BuyToken = async (
  ...
  let getRefAddress = localStorage.getItem("_DLT_REF_ADD");
...
export const SellToken = async (
  ...
  await dCenter.methods
    .sellTokens(
      JSON.stringify(1 * 1000000000).toString(),
      (5114954695279142).toString()
      // getVal
    )
    .send({
      from: userAccountAddress,
